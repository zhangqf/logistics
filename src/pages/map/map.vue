<template>
	<view class="map-container">
		<map style="width: 100%; height: 100vh;"  :scale="1" :markers="mapOptions.markers"
			:include-points="includePoints" :polyline='mapOptions.polyline'>
		</map>
		<view class="input-section-arrow">
			...
			<view class="top-info" v-if="isInputSectionVisible">
				<text class="time-range">{{departureTime}} — {{arrivalTime}}</text>
...
<script setup>
	...
	import aParking from '@/static/a_parking.png'
	import {
		onLoad
	...
	watch(() => mapOptions.value, (newVal) => {
		distance.value = newVal.distance || 0
	...
	const updateSpeed = () => {
		if (totalMinutes.value > 0) {
			// 速度 = 距离 / (总分钟数 / 60)
			speed.value = parseFloat((distance.value / (totalMinutes.value / 60)).toFixed(2))
	...
	const getDetail = async () => {
		try {
			...
			mapOptions.value = res
			const parsedPolyline = res.polyline
			includePoints.value = [parsedPolyline[0].points[0], parsedPolyline[0].points[parsedPolyline[0].points.length - 1]];
	...
	const calculateParkingPoints = () => {
		...
		const points = mapOptions.value.polyline[0].points;
		...
		let nextStopDistance = Math.random() * (maxContinuousDrivingTime - minContinuousDrivingTime) +
			minContinuousDrivingTime * speed / 60;
		const parkingPoints = [];
		const parkingTimes = {};
		...
		for (let i = 0; i < numPoints - 1; i++) {
			...
			if (currentDistance >= nextStopDistance) {
				...
				const parkingPointId = this.data.markers.length + parkingPoints.length;
