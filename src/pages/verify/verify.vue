...
<script>
import { ref } from 'vue'
import { verifyIdentity } from '@/api/verify'
...
export default {
  setup() {
    ...
    const formData = ref({
      name: userStore.userInfo?.real_name || '张三',
      idCard: userStore.userInfo?.id_card || '620523199008191392',
      licensePlate: userStore.userInfo?.license_plate || '甘A88293'
    })
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        const submitData = {
          ...
          ...(userRole.value === 'driver' ? { license_plate: formData.value.licensePlate } : {})
        }
